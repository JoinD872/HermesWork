...
## 当前 VPS 出图能力现状（2026-04-21 实测）

**已验证不可用：**
- `image_gen` 工具集：未启用（不在 toolsets 里）
- `baoyu-imagine`/`bun`：未安装
- MiniMax Image-01 API：auth.json 中无相关 key
- `MINIMAX_API_KEY` 环境变量：未设置（只有 sk-cp- 开头的 coding plan key，不含图片生成额度）

**可用方案：**
1. 用户自己在 Midjourney/DALL-E/Flux/MiniMax Image-01 等平台生成，提供详细英文 prompt
2. 如果 Hermes 部署到有图像生成能力的环境，重新启用 `image_gen` toolset + 安装 baoyu-imagine
